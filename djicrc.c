// DJI 16 and 32 bit CRC functions for OnBoard SDK.

#include <stdint.h>
#include <stdlib.h>

//! CRC16_IBM standard, divisor polynomial 0x8005
...
//! CRC32_Common standard, divisor polynomial 0x04C11DB7
...
uint16_t calculate_crc16(const uint8_t *data, size_t len)
{
    ...
    while (len--)
	acc = (acc >> 8) ^ crc_tab16[(acc & 0x0ff) ^ *data++];
    return acc;
}

uint32_t calculate_crc32(const uint8_t *data, size_t len)
{
    uint32_t acc = CRC_INIT;
    while (len--)
	acc = (acc >> 8) ^ crc_tab32[(acc & 0x0ff) ^ *data++];
    return acc;
}
